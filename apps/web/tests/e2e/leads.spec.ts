...
test("leads explorer supports filters, view toggles, selection, export, and empty state", async ({
  page,
}, testInfo) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Filter rail" }),
  ).toBeVisible();
  ...
  await page
    .getByRole("button", { name: /acme dental/i })
    .getByRole("checkbox")
    .click();
  await expect(page.getByText(/1 selected/i)).toBeVisible();

  await page
    .getByPlaceholder(/company, city, or domain/i)
    .fill("zzzz-no-match");
  await expect(
    page.getByText(/no leads match the current filters/i),
  ).toBeVisible();

  await page.getByRole("button", { name: /reset/i }).click();
  await expect(
    page.getByRole("button", { name: /acme dental/i }),
  ).toBeVisible();
