...
test("landing loads, major sections render, and navigation works", async ({
  page,
}, testInfo) => {
  ...
  await expect(
    page.getByRole("link", { name: "Request access", exact: true }).first(),
  ).toBeVisible();
  await expect(
    page.getByText(
      /discover, normalize, and score b2b leads with a deterministic pipeline/i,
    ),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", {
      name: /from blank query to approved outreach in one governed pipeline/i,
    }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", {
      name: /priced per seat for teams that take discovery seriously/i,
    }),
  ).toBeVisible();
...
test("sign-in page validates and can open the workspace", async ({
  page,
}, testInfo) => {
  await gotoPath(page, routes.login);

  await expect(
    page.getByText(/sign in to your leadscope workspace/i),
  ).toBeVisible();
  ...
  await expect(
    page.getByText(/operational lead intelligence desk/i),
  ).toBeVisible();
  await expectNoHorizontalOverflow(page);
});
...
test("signup page creates a workspace on all breakpoints", async ({ page }) => {
  ...
  await expect(
    page.getByText(/create your leadscope workspace/i),
  ).toBeVisible();
...
test("signup layout remains stable on tablet", async ({ page }, testInfo) => {
  test.skip(
    !isTabletProject(testInfo.project.name),
    "Tablet-only responsive capture",
  );
  await gotoPath(page, routes.signUp);
  await expect(
    page.getByText(/create your leadscope workspace/i),
...
test("forgot-password page renders unavailable notice and back-to-sign-in link", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText(/ask your workspace administrator/i),
  ).toBeVisible();
