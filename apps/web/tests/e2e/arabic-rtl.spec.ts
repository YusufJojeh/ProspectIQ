...
test("Arabic RTL shell and dashboard render without layout overflow", async ({
  page,
}, testInfo) => {
  await gotoPath(page, routes.dashboard);

  await expect(
    page.getByRole("heading", { name: /مكتب ذكاء العملاء التشغيلي/i }),
  ).toBeVisible({
    timeout: 15_000,
  });
  ...
  if (isMobileProject(testInfo.project.name)) {
    ...
    await expect(
      mobileNav.getByRole("link", { name: /العملاء المحتملون/i }),
    ).toBeVisible();
  }
});

test("Arabic RTL AI, leads, detail, and settings routes stay usable", async ({
  page,
}) => {
  await gotoPath(page, routes.aiAnalysis);
  await expect(
    page.getByRole("heading", { name: /توصيات مبنية على الأدلة/i }),
  ).toBeVisible({
    timeout: 15_000,
  });
  await expectArabicDocument(page);
