...
test("member cannot access admin-only governance routes", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "Team", exact: true })).toHaveCount(1);
  await expect(page.getByRole("link", { name: "Billing", exact: true })).toHaveCount(1);
  ...
  await expect(page.getByRole("heading", { name: /team users/i })).toBeVisible();
  await expect(page.getByText(/read-only access/i)).toBeVisible();
  await expect(page.getByRole("button", { name: /create team user/i })).toHaveCount(0);
...
test("inactive user cannot authenticate", async ({ page }) => {
  ...
  await expect(page.getByText(/an error occurred\. please try again\./i)).toBeVisible();
