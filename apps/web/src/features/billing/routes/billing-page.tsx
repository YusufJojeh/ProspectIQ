...
export function BillingPage() {
  ...
  const planMutation = useMutation({
    mutationFn: (planCode: string) =>
      changeSubscription({ plan_code: planCode, billing_cycle: billingCycle }),
  ...
  if (
    ...
    return (
      <QueryStateNotice
        tone="loading"
        title={t("billing.loadingTitle")}
        description={t("billing.loadingDescription")}
      />
    );
  }
  ...
  if (
    ...
    return (
      <QueryStateNotice
        ...
        description={t("billing.unavailableDescription")}
      />
    );
  }
  ...
  const teamUsersMetric = usageQuery.data.items.find(
          ...
          <div className="rounded-xl border border-border bg-card/50 p-4">
            ...
            <div className="mt-3 grid gap-1 text-sm text-muted-foreground">
              <p>
                ...
                {billingCycleLabel(t, subscriptionQuery.data.billing_cycle)}
              </p>
              <p>
                {t("billing.paymentMethod")}:{" "}
                {subscriptionQuery.data.simulated_payment_method}
              </p>
              <p>
                {t("billing.trialEnds")}:{" "}
                {formatDate(subscriptionQuery.data.trial_ends_at)}
              </p>
              <p>
                {t("billing.renews")}:{" "}
                {formatDate(subscriptionQuery.data.renews_at)}
              </p>
              <p>
                {t("billing.accessEnds")}:{" "}
                {formatDate(subscriptionQuery.data.ends_at)}
              </p>
              <p>
                {t("billing.canceledAt")}:{" "}
                {formatDate(subscriptionQuery.data.canceled_at)}
              </p>
            </div>
            {teamUsersMetric ? (
              <div className="mt-4 rounded-xl border border-border bg-background/70 p-3 text-sm text-muted-foreground">
                {t("billing.teamSeatsInUse")}: {teamUsersMetric.current_value} /{" "}
                {teamUsersMetric.limit_value ?? t("billing.unlimited")}
              </div>
            ) : null}
            {canManageBilling ? (
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {env.VITE_ENABLE_BILLING_SIMULATION && latestInvoice ? (
                    <Button
                      variant="outline"
                      onClick={() =>
                        simulateFailureMutation.mutate(latestInvoice.public_id)
                      }
              ...
              <QueryStateNotice
                tone="info"
                title={t("billing.ownerActionRequired")}
                description={t("billing.ownerActionDescription")}
              />
            )}
          </div>
            ...
            {plansQuery.data.items.map((plan) => (
              ...
              >
                ...
                <CardContent className="flex flex-col gap-3 text-sm text-muted-foreground">
                  <div>
                    {t("billing.searchesPerMonth")}:{" "}
                    {plan.limits.searches_per_month}
                  </div>
                  <div>
                    {t("billing.exportsPerMonth")}:{" "}
                    {plan.limits.exports_per_month}
                  </div>
                  <div>
                    {t("billing.aiRunsPerMonth")}:{" "}
                    {plan.limits.ai_scoring_runs_per_month}
                  </div>
                  <div>
                    {t("billing.outreachPerMonth")}:{" "}
                    {plan.limits.outreach_generations_per_month}
                  </div>
                  <div>
                    {t("billing.teamUsers")}: {plan.limits.max_team_users}
                  </div>
                  {canManageBilling ? (
                    <Button
                      variant={
                        plan.code === currentPlan ? "outline" : "default"
                      }
                      disabled={
                        planMutation.isPending ||
                        (plan.code === currentPlan &&
                          billingCycle === subscriptionQuery.data.billing_cycle)
                      }
