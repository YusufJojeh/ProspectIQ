...
from app.modules.provider_serpapi.models import LeadIdentity, LeadSourceRecord, ProviderNormalizedFact
...
class ProviderEvidenceRepository:
    def find_lead_by_identities(
        ...
        *,
        ...
        statement = (
            ...
            .where(Lead.workspace_id == workspace_id, LeadIdentity.workspace_id == workspace_id, or_(*conditions))
    ...
    def ensure_identities(
        ...
        *,
        ...
        existing = {
            ...
            for item in db.scalars(existing_statement)
        }
        to_add = [
            LeadIdentity(
    ...
    def add_normalized_fact(self, db: Session, fact: ProviderNormalizedFact) -> ProviderNormalizedFact:
        db.add(fact)
        db.commit()
        db.refresh(fact)
        return fact
    ...
    def get_latest_visibility(self, db: Session, lead_id: int) -> tuple[float | None, str | None]:
        ...
        confidence = float(visibility_confidence) if isinstance(visibility_confidence, (int, float)) else None
        return confidence, source if isinstance(source, str) else None

    def get_best_place_key(self, db: Session, lead_id: int) -> str | None:
        statement = (
            select(ProviderNormalizedFact)
            .where(
                ProviderNormalizedFact.lead_id == lead_id,
