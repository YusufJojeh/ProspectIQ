...
class AdminService:
    ...
    def get_active_scoring(self, db: Session, *, workspace_id: int) -> ActiveScoringConfigResponse:
        ...
        return ActiveScoringConfigResponse(active_version=self._to_scoring_response(version, creator))

    def list_scoring_versions(self, db: Session, *, workspace_id: int, limit: int = 50) -> ScoringConfigVersionListResponse:
        items = (
        ...
        users = {user.id: user for user in db.query(User).filter(User.id.in_([item.created_by_user_id for item in items])).all()}
        return ScoringConfigVersionListResponse(
            items=[self._to_scoring_response(item, users.get(item.created_by_user_id)) for item in items]
        )
    ...
    def activate_scoring_version(self, db: Session, *, workspace_id: int, version_public_id: str) -> ActiveScoringConfigResponse:
        version = (
            db.query(ScoringConfigVersion)
            .filter(ScoringConfigVersion.workspace_id == workspace_id, ScoringConfigVersion.public_id == version_public_id)
            .one_or_none()
        )
        if version is None:
    ...
    def get_provider_settings(self, db: Session, *, workspace_id: int) -> ProviderSettingsResponse:
        settings = db.query(ProviderSettings).filter(ProviderSettings.workspace_id == workspace_id).one_or_none()
        if settings is None:
    ...
    def _to_scoring_response(self, version: ScoringConfigVersion, creator: User | None) -> ScoringConfigVersionResponse:
